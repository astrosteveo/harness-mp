#!/bin/bash
# Bootstrap plugin-dev plugin into current project's .claude/ directory
...
echo "Bootstrapping plugin-dev plugin..."
...
if [ -d "$TEMPLATES_DIR/commands" ]; then
    cp -r "$TEMPLATES_DIR/commands/"* "$TARGET_DIR/commands/" 2>/dev/null || true
...
[ -d "$TARGET_DIR/commands" ] && echo "  Commands: $(find "$TARGET_DIR/commands" -name "*.md" -type f 2>/dev/null | wc -l)"
echo ""
echo "Done. Skills should now appear as slash commands."
echo ""
echo "Available agents: agent-creator, plugin-validator, skill-reviewer"
echo "Use these agents for AI-assisted plugin development."
